...
// dialog弹窗的显隐
const importModal = defineModel();
// 确定按钮的loading动画
const importLoading = defineModel('loading');

/**
 * 文件上传组件的扩展参数
 * downloadTemplate: 下载模板配置 (必须)
 *   单文件格式：
 *     name: 模板文件名称
 *     url: 模板文件下载链接
 *   多文件格式：
 *     name: 模板文件名称
 *     list: 模板文件数组,数组元素格式同单文件格式
 * isXlsx: 是否需要xlsx文件，默认为true
 *
 * submit: 文件上传完成后点击确定按钮的回调函数(必须)
 *   file: 上传的文件对象或OSS文件链接
...
const handleHttpRequest = async (file) => {
  try {
    if (!httpRequest) {
      ...
      uploadInfo.value.formFile = file.file;
      uploadInfo.value.fileType = 'file';
      return;
    }
    // 使用自定义上传方法
    const res = await httpRequest({ file: file.file });
    uploadInfo.value.fileName = res?.name || file.file.name;

    if (res?.url) {
      uploadInfo.value.fileType = 'url';
      uploadInfo.value.uploadUrl = res.url;
    } else {
      uploadInfo.value.fileType = 'file';
      uploadInfo.value.formFile = res?.file || file.file;
    }
  } catch (error) {
...
const resetUploadInfo = () => {
  uploadInfo.value.fileName = '';
  uploadInfo.value.uploadUrl = '';
  uploadInfo.value.formFile = null;
  uploadInfo.value.fileType = 'url';
};
